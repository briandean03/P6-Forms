...
import type { ActivityDiscipline } from '@/types/database'
...
interface ActivityDisciplineFormData {
  discipline_code: string
  discipline_name: string
}

const ITEMS_PER_PAGE = 15
type SortField = 'discipline_code' | 'discipline_name'
type SortDirection = 'asc' | 'desc'

export function ActivityDisciplineForm() {
  const [data, setData] = useState<ActivityDiscipline[]>([])
  ...
  const [editValues, setEditValues] = useState({ discipline_name: '' })
  const [showSaveConfirm, setShowSaveConfirm] = useState(false)
  const [showEditCancelConfirm, setShowEditCancelConfirm] = useState(false)
  const { notification, hideNotification, showSuccess, showError } = useNotification()

  const { register, handleSubmit, reset, formState: { errors, isDirty } } = useForm<ActivityDisciplineFormData>()
  ...
  const fetchData = async () => {
    setLoading(true)
    const { data: records, error } = await supabase.from('dbp6_activity_discipline').select('*').order('discipline_code', { ascending: true })
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(item =>
        item.discipline_code?.toLowerCase().includes(term) ||
  ...
  const onSubmit = async (formData: ActivityDisciplineFormData) => {
    setSaving(true)
    const { error } = await supabase.from('dbp6_activity_discipline').insert({
  ...
  const handleDelete = async (code: string) => {
    setDeleting(true)
    const { error } = await supabase.from('dbp6_activity_discipline').delete().eq('discipline_code', code)
    if (error) { showError('Failed to delete: ' + error.message) }
    else { setData(prev => prev.filter(item => item.discipline_code !== code)); showSuccess('Record deleted') }
    setDeleting(false); setDeleteConfirm(null)
  }

  const startEdit = (record: ActivityDiscipline) => {
    setEditingId(record.discipline_code)
    setEditValues({ discipline_name: record.discipline_name || '' })
  }

  const handleSaveEdit = async () => {
    if (!editingId) return
    const { error } = await supabase.from('dbp6_activity_discipline').update({
      discipline_name: editValues.discipline_name,
    } as never).eq('discipline_code', editingId)
    if (error) { showError('Failed to update: ' + error.message) }
    else {
      setData(prev => prev.map(item => item.discipline_code === editingId ? { ...item, discipline_name: editValues.discipline_name } : item))
  ...
  return (
    <div className="space-y-4">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-3 text-left">
                    <div className="flex items-center gap-1 text-xs font-medium text-gray-600 uppercase tracking-wide cursor-pointer hover:text-gray-800 whitespace-nowrap" onClick={() => handleSort('discipline_name')}>Name<SortIcon field="discipline_name" /></div>
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.length === 0 ? (
                  ...
                  const isEditing = editingId === record.discipline_code
                  if (isEditing) return (
                    <tr key={record.discipline_code} className="bg-amber-50">
                      <td className="px-3 py-2.5 text-sm font-mono font-medium text-gray-900 whitespace-nowrap">{record.discipline_code}</td>
                      <td className="px-3 py-2.5"><input type="text" value={editValues.discipline_name} onChange={e => setEditValues({ discipline_name: e.target.value })} className="w-full text-xs border border-amber-300 rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-amber-500" onKeyDown={e => { if (e.key === 'Enter') setShowSaveConfirm(true); if (e.key === 'Escape') setShowEditCancelConfirm(true) }} /></td>
                  ...
                  return (
                  <tr key={record.discipline_code} className="hover:bg-gray-50">
                    <td className="px-3 py-2.5 text-sm font-mono font-medium text-gray-900 whitespace-nowrap">{record.discipline_code}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCancelModal} title="Create Activity Discipline">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <FormField label="Discipline Code" type="text" {...register('discipline_code', { required: 'Code is required' })} error={errors.discipline_code?.message} />
          <FormField label="Discipline Name" type="text" {...register('discipline_name', { required: 'Name is required' })} error={errors.discipline_name?.message} />
      ...
      <ConfirmDialog isOpen={!!deleteConfirm} title="Delete Record" message="Permanently delete this discipline?" confirmLabel="Delete" loading={deleting}
