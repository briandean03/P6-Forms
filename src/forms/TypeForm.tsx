...
export function TypeForm({ projectId }: { projectId: string }) {
  ...
  const [editValues, setEditValues] = useState({ type_code: '', type_name: '' })
  ...
  const handleSaveEdit = async () => {
    ...
    if (error) { showError('Failed to update: ' + error.message) }
      ...
      setData(prev => prev.map(item => item.id === editingId
        ? { ...item, type_code: editValues.type_code ? parseFloat(editValues.type_code) : null, type_name: editValues.type_name || null }
        : item))
